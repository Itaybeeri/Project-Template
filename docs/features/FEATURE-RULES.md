...
4. One feature = one branch = one PR into the default branch.
   **NON-NEGOTIABLE merge gate:**
   ...
   - **No release note → no PR.** Every PR carries `docs/releases/NNNN-slug.md` plus the HTML
     ...
     automatic and never waived: a trivial fix or a docs-only change gets a two-bullet note.
     The generator's own validation is part of the green gate — a dead ref, a duplicate note
     number, or a missing field fails it.
